...
constexpr bool isAsciiHexDigit(Rune rune) {
    return isAsciiDecDigit(rune)
        or (rune >= 'a' and rune <= 'f')
        or (rune >= 'A' and rune <= 'F');
}
...
constexpr bool isUnicodeNonCharacter(Rune rune) {
    return isUnicode(rune)
       and ((rune >= 0xFDD0 and rune <= 0xFDEF)
            or ((rune & 0xFFFE) == 0xFFFE)
            or ((rune & 0xFFFF) == 0xFFFF));
}

// MARK: Character Width ------------------------------------------------------------

constexpr u8 numDigits(usize n, usize base = 10) {
    u8 digits = 1;
