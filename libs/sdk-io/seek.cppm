...
struct Seek {
    ...
    Seek(Whence whence, isize offset = 0) : whence(whence), offset(offset) { }

    constexpr usize apply(usize current, usize size) const {
        switch (whence) {
            case Whence::BEGIN: return offset;

            case Whence::CURRENT: return current + offset;

            case Whence::END: return size - offset;
        }
    }
