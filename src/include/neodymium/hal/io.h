...
struct PortIo : public Io {
    ...
    Res<usize> in(usize offset, usize size) override {
        ...
        switch (size) {
            case 1:  return in8(addr).map<usize>();
            case 2:  return in16(addr).map<usize>();
            case 4:  return in32(addr).map<usize>();
            case 8:  return in64(addr).map<usize>();
    ...
    Res<> out(usize offset, usize size, usize val) override {
        ...
        switch (size) {
            case 1:  return out8(addr, (u8) val);
            case 2:  return out16(addr, (u16) val);
            case 4:  return out32(addr, (u32) val);
            case 8:  return out64(addr, (u64) val);
